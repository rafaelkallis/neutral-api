...
import { SumOfPairsCliquismComputer } from './SumOfPairsCliquismComputer';

describe(CliquismComputer.name, () => {
  ...
  describe(SumOfPairsCliquismComputer.name, () => {
    beforeEach(() => {
      cliquismComputer = new SumOfPairsCliquismComputer();
    });

    test('cycle', () => {
      const result = cliquismComputer.compute(cycleProject);
      expect(result.ofReviewTopic(reviewTopic.id).value).toBeCloseTo(0);
    });

    test('clusters', () => {
      const result = cliquismComputer.compute(clusterProject);
      expect(result.ofReviewTopic(reviewTopic.id).value).toBeCloseTo(39984);
    });

    test('one did everything', () => {
      const result = cliquismComputer.compute(oneDidItAllProject);
      expect(result.ofReviewTopic(reviewTopic.id).value).toBeCloseTo(0);
    });
  });
});
